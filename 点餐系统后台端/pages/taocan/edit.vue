    ...
    methods: {
      ...
      onShangpinChange(shangpinId, index) {
        ...
        if (option) {
          ...
          const priceStr = option.text.split(' ¥')[1]
      ...
      async getDetail(id) {
        ...
        try {
          ...
          const res = await db.collection('taocan').doc(id).field('name,type,price,original_price,image,description,stock,status,sale_time_start,sale_time_end,sort,items').get()
          ...
          if (data) {
            this.formData = {
              ...
              original_price: data.original_price !== undefined ? String(data.original_price) : '',
              image: data.image || '',
              ...
              items: (data.items || []).map(item => ({
                shangpin_id: item.shangpin_id || null,
      ...
      async submitForm(value) {
        try {
          ...
          const res = await taocanCo.updateTaocan({
            id: this.formDataId,
            name: value.name,
            type: value.type,
            price: value.price,
