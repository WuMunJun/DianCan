    ...
    methods: {
      ...
      async getDetail(id) {
        ...
        try {
          ...
          const res = await db.collection('shangpin').doc(id).field('name,fenlei_id,fenlei_name,price,original_price,image,description,unit,stock,status,sale_time_start,sale_time_end,sort,tags').get()
          const data = res.result.data[0]
          if (data) {
            this.formData = {
              name: data.name || '',
              fenlei_id: data.fenlei_id || null,
      ...
      async submitForm(value) {
        try {
          const shangpinCo = uniCloud.importObject('shangpin')
          const res = await shangpinCo.updateShangpin({
            id: this.formDataId,
            name: value.name,
