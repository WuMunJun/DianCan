  ...
  <view class="container">
    <view class="filter-bar">
      <view class="filter-row">
        <uni-data-select v-model="filterFenlei" :localdata="fenleiOptions" placeholder="全部分类" @change="onFilterChange" class="filter-select"></uni-data-select>
      ...
      <view class="filter-row">
        <uni-easyinput prefixIcon="search" v-model="keyword" placeholder="搜索商品名称" @confirm="search" @clear="search" class="filter-input"></uni-easyinput>
      </view>
    </view>
      ...
      <view v-for="(item, index) in dataList" :key="index" class="list-item" @click="handleItemClick(item._id)">
        <view class="item-left">
          ...
          <view v-else class="item-image placeholder-img">
            <uni-icons type="image" size="28" color="#ccc"></uni-icons>
          </view>
          <view class="item-info">
            <text class="item-name">{{item.name}}</text>
            <text class="item-fenlei">{{item.fenlei_name || '未分类'}}</text>
            ...
            <view class="item-meta">
              ...
              <text v-if="item.sale_time_start" class="item-time">{{item.sale_time_start}}-{{item.sale_time_end}}</text>
            </view>
          </view>
        </view>
    ...
    methods: {
      async loadFenleiList() {
        try {
          ...
          if (res.errCode === 0) {
            this.fenleiOptions = res.data.map(item => ({
              value: item._id,
              text: item.name
            }))
          }
      ...
      async loadData(clear = false, callback) {
        ...
        try {
          ...
          if (this.filterFenlei) {
            where.fenlei_id = this.filterFenlei
          }
          ...
          const res = await query.field('name,fenlei_id,fenlei_name,price,original_price,image,unit,stock,status,sale_time_start,sale_time_end,sort')
            .orderBy('sort', 'asc')
      ...
      async toggleStatus(item) {
        try {
          const shangpinCo = uniCloud.importObject('shangpin')
          const res = await shangpinCo.toggleShangpinStatus(item._id)
          if (res.errCode === 0) {
